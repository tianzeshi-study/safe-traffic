...
impl TrafficDaemon {
    ...
    async fn process_request(data: &[u8], firewall: &Arc<Firewall>) -> Result<Response> {
        ...
        let response_data = match request {
            Request::Limit { ip, kbps, burst } => {
                match firewall.limit(ip, kbps, burst).await {
                    Ok(rule_id) => {
            ...
            Request::Ban { ip, seconds } => {
                match firewall.ban(ip, seconds).await {
                    Ok(rule_id) => {
            ...
            Request::Unban { rule_id } => {
                match firewall.unban(&rule_id).await {
                    Ok(_) => {
            ...
            Request::CleanupExpired => {
                match firewall.cleanup_expired().await {
                    Ok(expired_rules) => {
            ...
            Request::GetActiveRules => {
                match firewall.get_active_rules().await {
                    Ok(rules) => {
            ...
            Request::GetSystemRules => {
                match firewall.get_system_rules().await {
                    Ok(rules_output) => {
            ...
            Request::BatchBan { ips, seconds } => {
                match firewall.batch_ban(ips.clone(), seconds).await {
                    Ok(rule_ids) => {
                        info!("Successfully batch banned {} IPs for {} seconds", ips.len(), seconds);
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        if tokio::fs::metadata(&self.socket_path).await.is_ok() {
            tokio::fs::remove_file(&self.socket_path).await
                .with_context(|| format!("Failed to remove socket during shutdown: {}", self.socket_path))?;
        }
